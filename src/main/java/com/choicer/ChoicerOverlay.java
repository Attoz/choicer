...
/**
 * Alternate overlay used when Choicer mode is enabled.
...
public class ChoicerOverlay extends Overlay implements RollOverlay
{
    ...
    List<Rectangle> getCurrentHitboxes()
    {
        ...
        int boundsX = bounds == null ? 0 : bounds.x;
        int boundsY = bounds == null ? 0 : bounds.y;
        synchronized (columnHitboxes)
        {
            List<Rectangle> copy = new ArrayList<>(columnHitboxes.size());
            for (Rectangle rect : columnHitboxes)
            {
                // Tests treat hitboxes as local to the overlay bounds.
                copy.add(new Rectangle(rect.x - boundsX, rect.y - boundsY, rect.width, rect.height));
            }
            return copy;
        }
    }
