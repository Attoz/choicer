...
public class ChoicerPanel extends PluginPanel {
    ...
    // View selection row: 3 buttons (swap, filter unlocked-not-rolled, filter
    // unlocked-and-rolled)
    ...
    /**
     * Main update routine: filters the active set (Unlocked or Rolled), applies
     * search text and filter toggles,
     * updates the single count label, and then builds the item list without
     * trailing gaps.
     */
    ...
    /**
     * Asynchronously retrieves the item name for a given item ID and passes it to
     * the callback.
     *
     * @param itemId   The item ID.
