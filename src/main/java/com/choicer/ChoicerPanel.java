...
public class ChoicerPanel extends PluginPanel
{
    ...
    private void applyFilterIcons()
    {
        ImageIcon swap = loadButtonIcon("/com/choicer/arrow_rotate.png", 18, ACCENT);
        ImageIcon unlockedOnly = loadButtonIcon("/com/choicer/lock_open.png", 18, ACCENT);
        ImageIcon unlockedAndRolled = loadButtonIcon("/com/choicer/collection.png", 18, ACCENT);
        if (swap != null)
        {
            swapViewButton.setIcon(swap);
            swapViewButton.setText("");
        }
        if (unlockedOnly != null)
        {
            filterUnlockedNotRolledButton.setIcon(unlockedOnly);
            filterUnlockedNotRolledButton.setText("");
        }
        if (unlockedAndRolled != null)
        {
            filterUnlockedAndRolledButton.setIcon(unlockedAndRolled);
            filterUnlockedAndRolledButton.setText("");
        }
    }

    private ImageIcon loadButtonIcon(String path, int size, Color tint)
    {
        try
        {
            ...
            g2.dispose();
            BufferedImage tinted = tint != null ? tintImage(scaled, tint) : scaled;
    ...
    private JPanel buildSearchBar()
    {
        ...
        ImageIcon searchPng = loadButtonIcon("/com/choicer/search.png", 16, ACCENT);
        if (searchPng != null)
        {
            searchIcon.setIcon(searchPng);
        }
        ...
        ImageIcon clearPng = loadButtonIcon("/com/choicer/clear.png", 14, ACCENT);
        if (clearPng != null)
        {
            clearLabel.setIcon(clearPng);
        }
